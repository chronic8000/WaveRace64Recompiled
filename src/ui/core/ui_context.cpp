...
// Suppress try_close_current_context while create_context loads an RML file.
thread_local bool suppress_context_close_for_load = false;
...
recompui::ContextId recompui::create_context(const std::filesystem::path& path) {
    ...
    // Custom element constructors and handlers call get_current_context() during
    // LoadDocument. ProcessEvent also calls try_close_current_context(), which used
    // to clear opened_context before document assignment.
    new_context.open();
    suppress_context_close_for_load = true;
    Rml::ElementDocument* doc = recompui::load_document(path);
    struct LoadGuard {
        ~LoadGuard() { suppress_context_close_for_load = false; }
    } load_guard;
    if (doc == nullptr) {
        context_error(new_context, ContextErrorType::InternalError);
    }

    if (opened_context_id != new_context) {
        new_context.open();
    }
...
recompui::ContextId recompui::try_close_current_context() {
    if (suppress_context_close_for_load) {
