...
static std::atomic<bool> g_game_loading_dismissed{false};
static std::atomic<bool> g_game_gfx_submitted{false};

void recompui::reset_game_loading_overlay() {
    g_game_loading_dismissed.store(false, std::memory_order_release);
    g_game_gfx_submitted.store(false, std::memory_order_release);
    wr64::reset_nonempty_swapchain_presented();
}

void recompui::notify_game_gfx_submitted() {
    g_game_gfx_submitted.store(true, std::memory_order_release);
}

bool recompui::is_game_gfx_submitted() {
    return g_game_gfx_submitted.load(std::memory_order_acquire);
}

bool recompui::is_nonempty_game_frame_presented() {
    return wr64::nonempty_swapchain_presented();
}

void recompui::dismiss_game_loading_overlay() {
    g_game_loading_dismissed.store(true, std::memory_order_release);
    recompui::close_prompt();
}

bool recompui::is_game_loading_dismissed() {
    return g_game_loading_dismissed.load(std::memory_order_acquire);
}
