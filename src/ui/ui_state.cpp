...
#if defined(_WIN32)
static DWORD sdl_window_thread_id = 0;
#endif

namespace {
#if defined(_WIN32)
    // Single-slot marshaller: only one thread may wait in run_on_main_thread at a time.
    std::mutex main_thread_task_mutex;
    std::condition_variable main_thread_task_cv;
    std::function<void()> main_thread_task_fn;
    bool main_thread_task_pending = false;
    bool main_thread_task_done = false;
    std::exception_ptr main_thread_task_exception;

    void enqueue_main_thread_task(std::function<void()> fn) {
        {
            std::lock_guard lock{ main_thread_task_mutex };
            assert(!main_thread_task_pending && "concurrent run_on_main_thread call");
            main_thread_task_fn = std::move(fn);
            main_thread_task_pending = true;
            main_thread_task_done = false;
            main_thread_task_exception = nullptr;
        }

        std::unique_lock lock{ main_thread_task_mutex };
        main_thread_task_cv.wait(lock, [] { return main_thread_task_done; });

        if (main_thread_task_exception) {
            std::rethrow_exception(main_thread_task_exception);
        }
    }

    template<typename Fn>
    auto run_on_main_thread(Fn&& fn) -> std::invoke_result_t<Fn> {
        using Result = std::invoke_result_t<Fn>;

        if (sdl_window_thread_id != 0 && GetCurrentThreadId() == sdl_window_thread_id) {
        ...
        enqueue_main_thread_task([task]() { (*task)(); });
...
public:
    ...
    UIState(SDL_Window* window, RT64::RenderInterface* interface, RT64::RenderDevice* device) {
        ...
        SystemInterface_SDL* system_interface_ptr = system_interface.get();
        SDL_Window* sdl_window = window;
        run_on_main_thread([system_interface_ptr, sdl_window]() {
            system_interface_ptr->SetWindow(sdl_window);
        });
        if (zelda64::startup_verbose_enabled()) {
            fprintf(stderr, "[startup] UIState: SetWindow done\n");
            fflush(stderr);
        }

        render_interface.init(interface, device);
        if (zelda64::startup_verbose_enabled()) {
            fprintf(stderr, "[startup] UIState: render_interface.init done\n");
            fflush(stderr);
        }
        ...
        Rml::Initialise();
        if (zelda64::startup_verbose_enabled()) {
            fprintf(stderr, "[startup] UIState: Rml::Initialise done\n");
            fflush(stderr);
        }
        ...
        const auto [width, height] = run_on_main_thread([sdl_window]() {
            int w = 0;
            int h = 0;
        ...
        if (zelda64::startup_verbose_enabled()) {
            fprintf(stderr, "[startup] UIState: GetWindowSize done (%d x %d)\n", width, height);
            fflush(stderr);
        }
...
static std::pair<int, int> get_window_size_pixels() {
    SDL_Window* window_ptr = window;
    return run_on_main_thread([window_ptr]() {
        int w = 0;
        int h = 0;
        SDL_GetWindowSizeInPixels(window_ptr, &w, &h);
        return std::pair{ w, h };
    });
}

void recompui::get_window_size(int& width, int& height) {
    const auto [w, h] = get_window_size_pixels();
    width = w;
    height = h;
}
...
void recompui::set_sdl_window_thread_id(uint32_t thread_id) {
#if defined(_WIN32)
    sdl_window_thread_id = thread_id;
#else
    (void)thread_id;
#endif
}

void recompui::process_main_thread_tasks() {
#if defined(_WIN32)
    std::function<void()> fn;
    {
        std::lock_guard lock{ main_thread_task_mutex };
        if (!main_thread_task_pending) {
            return;
        }
        fn = std::move(main_thread_task_fn);
        main_thread_task_pending = false;
    }

    if (zelda64::startup_verbose_enabled()) {
        fprintf(stderr, "[startup] process_main_thread_tasks: executing\n");
        fflush(stderr);
    }
    ...
    try {
        ...
        std::lock_guard lock{ main_thread_task_mutex };
        main_thread_task_exception = std::current_exception();
    }

    {
        std::lock_guard lock{ main_thread_task_mutex };
        main_thread_task_done = true;
    }
    main_thread_task_cv.notify_one();
#endif
}
...
#endif
    if (zelda64::startup_verbose_enabled()) {
        fprintf(stderr, "[startup] init_hook: entered\n");
        fflush(stderr);
    }
    try {
        ui_state = std::make_unique<UIState>(window, interface, device);
        ui_state->create_menus();
        ui_state->render_interface.sync_upload_pending_textures();
    } catch (const std::exception& e) {
        fprintf(stderr, "UI init failed: %s\n", e.what());
        fflush(stderr);
        throw;
    }
    if (zelda64::startup_verbose_enabled()) {
        fprintf(stderr, "[startup] init_hook: exited\n");
        fflush(stderr);
    }
}
...
void draw_hook(RT64::RenderCommandList* command_list, RT64::RenderFramebuffer* swap_chain_framebuffer) {
    static std::atomic<uint32_t> draw_frame_count{ 0 };
    const uint32_t frame_index = draw_frame_count.fetch_add(1) + 1;
    if (frame_index <= 5 && zelda64::startup_verbose_enabled()) {
        fprintf(stderr, "[startup] draw_hook: frame %u begin\n", frame_index);
        fflush(stderr);
    }

    auto draw_step = [&](const char* step) {
        if (frame_index <= 5 && zelda64::startup_verbose_enabled()) {
            fprintf(stderr, "[startup] draw_hook: frame %u %s\n", frame_index, step);
            fflush(stderr);
        }
    };
    ...
    recompui::try_run_queued_auto_start(frame_index);

    static std::chrono::steady_clock::time_point loading_overlay_since{};
    static bool loading_overlay_timer_armed = false;
    if (ultramodern::is_game_started() && !loading_overlay_timer_armed) {
        loading_overlay_since = std::chrono::steady_clock::now();
        loading_overlay_timer_armed = true;
    }

    // Dismiss the loading overlay once a real frame is on screen, or after a boot timeout.
    if (ultramodern::is_game_started() && !recompui::is_game_loading_dismissed()) {
        const bool gfx_ready = recompui::is_game_gfx_submitted();
        const bool frame_ready = recompui::is_nonempty_game_frame_presented();
        const bool boot_timeout =
            loading_overlay_timer_armed &&
            std::chrono::steady_clock::now() - loading_overlay_since > std::chrono::seconds(15);
        if (frame_ready || (gfx_ready && boot_timeout)) {
            recompui::dismiss_game_loading_overlay();
        }
    }

    // Return to the launcher if no menu is open and the game isn't started.
    // After Start Game keep a loading overlay visible until the game renders.
    if (!recompui::is_any_context_shown()) {
        if (!ultramodern::is_game_started()) {
            recompui::show_context(recompui::get_launcher_context_id(), "");
        } else if (!recompui::is_prompt_open() && !recompui::is_game_loading_dismissed()) {
            recompui::open_notification("Loading", "Starting Wave Race 64...");
        }
    }
    draw_step("after show_context");

    std::lock_guard lock{ ui_state_mutex };
    draw_step("after lock");
    ...
    if (recompui::is_any_context_shown()) {
        draw_step("before update_contexts");
        ui_state->update_contexts();
        draw_step("after update_contexts");
        ...
        draw_step("before render start");
        ui_state->render_interface.start(command_list, width, height);
        draw_step("after render start");
        ...
        draw_step("before Render");
        try {
            ui_state->context->Render();
        } catch (const std::exception& e) {
            fprintf(stderr, "UI render failed: %s\n", e.what());
            fflush(stderr);
            throw;
        }
        draw_step("after Render");
        ui_state->render_interface.end(command_list, swap_chain_framebuffer);

        static std::atomic<bool> traced_first_render{ false };
        if (!traced_first_render.exchange(true) && zelda64::startup_verbose_enabled()) {
            fprintf(stderr, "[startup] draw_hook: first render complete\n");
            fflush(stderr);
        }
    }

    if (frame_index <= 5 && zelda64::startup_verbose_enabled()) {
        fprintf(stderr, "[startup] draw_hook: frame %u done\n", frame_index);
        fflush(stderr);
    }
    } catch (const std::exception& e) {
        fprintf(stderr, "draw_hook failed on frame %u: %s\n", frame_index, e.what());
        fflush(stderr);
        throw;
    } catch (...) {
        fprintf(stderr, "draw_hook failed on frame %u: unknown exception\n", frame_index);
        fflush(stderr);
        throw;
    }
}
...
void deinit_hook() {
    if (zelda64::startup_verbose_enabled()) {
        fprintf(stderr, "[startup] deinit_hook: RT64 shutting down UI\n");
        fflush(stderr);
    }
