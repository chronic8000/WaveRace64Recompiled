...
static bool handle_window_close_request() {
    if (!ultramodern::is_game_started()) {
        ultramodern::quit("SDL window close before game started");
        return true;
    }

    // After Start Game the launcher is hidden; if no overlay is visible the quit
    // prompt cannot be shown and the window appears to ignore the close button.
    if (!recompui::is_any_context_shown()) {
        ultramodern::quit("SDL window close during game (no UI overlay visible)");
        return true;
    }
...
bool sdl_event_filter(void* userdata, SDL_Event* event) {
    ...
    case SDL_EventType::SDL_QUIT:
        return handle_window_close_request();
...
void recomp::handle_events() {
    ...
    while (SDL_PollEvent(&cur_event) && !exited) {
        ...
        // Lock the cursor once the game is running and a real frame is on screen (not during black boot).
        bool cursor_locked = (recomp::get_mouse_sensitivity() != 0) && !recomp::game_input_disabled() &&
            ultramodern::is_game_started() && recompui::is_nonempty_game_frame_presented();
