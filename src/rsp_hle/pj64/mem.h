...
static inline uint8_t * hle_ptr_u8(const unsigned char * buffer, unsigned address)
{
    return (uint8_t *)(buffer + (address ^ S8));
}

static inline uint16_t * hle_ptr_u16(const unsigned char * buffer, unsigned address)
{
    assert((address & 1) == 0);
    return (uint16_t *)(buffer + (address ^ S16));
}

static inline uint32_t * hle_ptr_u32(const unsigned char * buffer, unsigned address)
{
    assert((address & 3) == 0);
...
{
    return hle_ptr_u32(hle->dmem(), address & 0xfff);
}
...
{
    return hle_ptr_u8(hle->dram(), address & 0xffffff);
}

static inline uint16_t * dram_u16(CHle * hle, uint32_t address)
{
    return hle_ptr_u16(hle->dram(), address & 0xffffff);
}

static inline uint32_t * dram_u32(CHle * hle, uint32_t address)
{
    return hle_ptr_u32(hle->dram(), address & 0xffffff);
}
