...
#if defined(_WIN32)
    const DWORD window_thread_id = GetCurrentThreadId();
    recompui::set_sdl_window_thread_id(window_thread_id);
    if (zelda64::startup_verbose_enabled()) {
        fprintf(stderr, "[startup] create_window: done\n");
        fflush(stderr);
    }
    return ultramodern::renderer::WindowHandle{ wmInfo.info.win.window, window_thread_id };
...
void update_gfx(void*) {
    recompui::process_main_thread_tasks();
...
extern RspUcodeFunc wr64_aspMain;
...
extern "C" void wr64_boot_entrypoint(uint8_t* rdram, recomp_context* ctx) {
    wr64::prepare_boot_entrypoint(rdram);
...
static void wr64_vi_callback() {
    recomp::update_rumble();
    if (wr64_last_rdram != nullptr) {
        wr64_on_vi_retrace(wr64_last_rdram);
    }
}
...
std::vector<recomp::GameEntry> supported_games = {
    {
        .rom_hash = 0x2B675E2250A604FCULL,
        .internal_name = "WAVE RACE 64",
        .game_id = u8"wr64.us.revA",
        .mod_game_id = "wr64",
        .save_type = recomp::SaveType::AllowAll,
        .is_enabled = true,
        .decompression_routine = zelda64::decompress_wr64,
        .has_compressed_code = false,
        .entrypoint_address = get_entrypoint_address(),
        .entrypoint = wr64_boot_entrypoint,
        .thread_create_callback = wr64::on_thread_create,
        .on_init_callback = wr64::on_game_init,
        .host_sleep_after_entrypoint = true,
        .on_entrypoint_returned_callback = wr64::notify_entrypoint_returned,
    },
    {
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--verbose-startup") == 0) {
            zelda64::set_startup_verbose(true);
        }
    }
    // --show-console implies startup tracing for launcher bring-up.
    for (int i = 1; i < argc; i++) {
    ...
    SetUnhandledExceptionFilter([](EXCEPTION_POINTERS* info) -> LONG {
        ...
        ultramodern::boot_log("[startup] unhandled exception: 0x%08lx at %p\n", code, address);
        std::fprintf(stderr, "[startup] unhandled exception: 0x%08lx at %p\n", code, address);
        std::fflush(stderr);
        if (info && info->ExceptionRecord && info->ExceptionRecord->NumberParameters >= 1) {
            ultramodern::boot_log("[startup]   access type: %llu (0=read, 1=write)\n",
        ...
        if (info && info->ExceptionRecord && info->ExceptionRecord->NumberParameters >= 2) {
            ...
            ultramodern::boot_log("[startup]   fault address: 0x%llx\n",
                static_cast<unsigned long long>(fault));
            extern uint8_t* wr64_last_rdram;
            if (wr64_last_rdram != nullptr && fault >= reinterpret_cast<uintptr_t>(wr64_last_rdram)) {
                const uintptr_t offset = fault - reinterpret_cast<uintptr_t>(wr64_last_rdram);
                const uint32_t rdram_off = static_cast<uint32_t>(offset & 0x00FFFFFFu);
                const uint64_t seg = 0x80000000ULL + rdram_off;
                ultramodern::boot_log("[startup]   approx vaddr: 0x%08llx (rdram+0x%llx)\n",
                    static_cast<unsigned long long>(seg),
                    static_cast<unsigned long long>(offset));
                if (offset >= 0x40000000ULL && rdram_off < 0x00800000u) {
                    ultramodern::boot_log(
                        "[startup]   hint: host offset 0x%llx looks like sign-extension bug; "
                        "masked RDRAM offset 0x%06x (vaddr ~0x%08llx)\n",
                        static_cast<unsigned long long>(offset),
                        rdram_off,
                        static_cast<unsigned long long>(seg));
                } else if (offset == 0x40000000ULL) {
                    ultramodern::boot_log(
                        "[startup]   hint: host offset 0x40000000 is the 1 GiB sandbox guard page "
                        "(invalid KSEG2+ vaddr 0xC0000000 or unmapped pointer)\n");
                }
                if (seg < 0x80000000ULL) {
                    ultramodern::boot_log(
                        "[startup]   hint: KUSEG vaddr (0x%08llx) — float/int register used as pointer "
                        "(stack frame corruption from merged recompiled functions?)\n",
                        static_cast<unsigned long long>(seg));
                } else if (offset >= 0x40000000ULL) {
                    ultramodern::boot_log(
                        "[startup]   hint: rdram offset exceeds 1 GiB sandbox — corrupted MIPS pointer\n");
                } else if (seg >= 0x802D4000ULL && seg < 0x8036A000ULL) {
                    ultramodern::boot_log(
                        "[startup]   hint: vaddr 0x%08llx lies inside mode-3 FB0 "
                        "(0x802D4000-0x8036A000) — likely pixel/FBO data used as pointer\n",
                        static_cast<unsigned long long>(seg));
                }
                if (offset == 0x12) {
                    ultramodern::boot_log("[startup]   hint: MEM_H(+0x10,NULL) also faults at rdram+0x12 (endian xor)\n");
                }
                if (offset == 0x4) {
                    ultramodern::boot_log("[startup]   hint: likely lw +0x4(NULL) in timer/virt-count list walk (static_0_800CB220)\n");
                }
                if (offset == 0xC2C80000) {
                    ultramodern::boot_log(
                        "[startup]   hint: offset 0xC2C80000 is IEEE754 -100.0f — float used as pointer, not missing DMA\n");
                }
            }
            extern uint8_t* wr64_last_rdram;
            if (wr64_last_rdram != nullptr) {
                auto rd32 = [&](uint32_t vaddr) -> uint32_t {
                    return *reinterpret_cast<uint32_t*>(
                        wr64_last_rdram + wr64_kseg0_to_rdram_offset(vaddr));
                };
                ultramodern::boot_log(
                    "[startup]   wr64 globals: gGameState=0x%08X (byte=%u) D_801CE638=0x%08X "
                    "D_801CE63C=0x%08X D_800D45DC={0x%08X,0x%08X} D_800D45D8=%u\n",
                    rd32(0x800DAB24u),
                    rd32(0x800DAB24u) & 0xFFu,
                    rd32(0x801CE638u),
                    rd32(0x801CE63Cu),
                    rd32(0x800D45DCu),
                    rd32(0x800D45E0u),
        ...
        if (GetThreadDescription(GetCurrentThread(), &thread_name) == S_OK && thread_name != nullptr) {
            fwprintf(stderr, L"[startup]   thread: %ls\n", thread_name);
            LocalFree(thread_name);
        }
        if (FILE* boot_log = std::fopen("wr64_boot.log", "a")) {
            std::fprintf(boot_log, "[startup] unhandled exception: 0x%08lx at %p\n", code, address);
            if (info && info->ExceptionRecord && info->ExceptionRecord->NumberParameters >= 2) {
                std::fprintf(boot_log, "[startup]   fault address: 0x%llx\n",
                    static_cast<unsigned long long>(info->ExceptionRecord->ExceptionInformation[1]));
            }
            std::fclose(boot_log);
        }
        ExitProcess(code != 0 ? code : 1);
        return EXCEPTION_EXECUTE_HANDLER;
    });

    // Optional file logging. PowerShell's `2> file` often captures nothing from GUI subsystem exes.
    ...
    if (log_path == nullptr && std::filesystem::exists("portable.txt")) {
        log_path = "wr64_boot.log";
    }
    if (log_path != nullptr) {
        if (std::freopen(log_path, "w", stderr) != nullptr) {
            std::setvbuf(stderr, nullptr, _IONBF, 0);
            std::fprintf(stderr, "[startup] logging to %s\n", log_path);
            std::fflush(stderr);
        }
    }
    ...
    // Mirror logs to a console when requested, but keep --log-file as the primary stderr target.
    if (show_console) {
    ...
    // Change to a font that supports Japanese characters (only when a console exists).
    if (HANDLE con_out = GetStdHandle(STD_OUTPUT_HANDLE); con_out != nullptr && con_out != INVALID_HANDLE_VALUE) {
        ...
        SetCurrentConsoleFontEx(con_out, FALSE, &cfi);
    }
#endif
    ...
    wr64::load_boot_config();
    const bool stock_boot = wr64::stock_boot_enabled();
    if (stock_boot) {
        recomp::GameEntry& wr = supported_games[0];
        wr.entrypoint = recomp_entrypoint;
        wr.thread_create_callback = nullptr;
        wr.on_init_callback = wr64::on_game_init;
        wr.host_sleep_after_entrypoint = false;
        fprintf(stderr,
            "[boot] stock_boot=1: dino-like runtime (no wr64 HLE patches, stock VI, no vi_pulse)\n");
        fflush(stderr);
    }
    ...
    recomp::try_import_bundled_roms(zelda64::get_program_path());
    ...
    ultramodern::events::callbacks_t thread_callbacks{
        .vi_callback = stock_boot ? nullptr : wr64_vi_callback,
        .gfx_init_callback = recompui::update_supported_options,
        .rsp_event_delivery_callback = stock_boot ? nullptr : wr64_wake_main_after_rsp_event,
    };
